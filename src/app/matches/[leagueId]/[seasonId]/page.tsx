...
function Matches({
  ...
  params: { leagueId: number; seasonId: number };
}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [leaguesResponse, seasonsResponse, leagueSeasonResponse] =
          await Promise.all([
            getAllLeagues(),
            getAllSeasons(),
        ...
        if (leagueSeasonResponse && leagueSeasonResponse.id) {
          const matchesResponse = await getMatchByLeagueSeasonId(
            leagueSeasonResponse.id,
            {
              include: [
                {
                  relation: "homeClub",
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center">
                  <select
                    ...
                    onChange={(e) =>
                      router.push(
                        `/matches/${e.target.value}/${params.seasonId}`
                      )
                    }
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      router.push(
                        `/matches/${params.leagueId}/${e.target.value}`
                      )
                    }
